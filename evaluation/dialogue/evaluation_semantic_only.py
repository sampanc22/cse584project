...
def run_dialogue_semantic_only_batch(
    batch_idx: int,
    ...
    print(f"Running semantic-only dialogue batch {batch_idx} with {len(batch_trajectories)} trajectories")
    ...
    for i, req in enumerate(stream, start=1):
        ...
        print(
            f"\n[DIALOGUE SEMANTIC-ONLY BATCH {batch_idx} {i}/{len(stream)}] "
        ...
        pred, hit, hit_gold_valid = runner.answer_semantic_only(
            question=question,
            history=history,
    ...
    output_json_path = f"tmp/dialogue_semantic_only_batch_{batch_idx}.json"
...
if __name__ == "__main__":
    ...
    run_dialogue_semantic_only_batch(
