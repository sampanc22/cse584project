...
STRICTNESS = "intent_domain"


def run_dialogue_semantic_plus_validity_batch(
    batch_idx: int,
    ...
    print(f"Running dialogue semantic+validity ({STRICTNESS}) batch {batch_idx} with {len(batch_trajectories)} trajectories")
    ...
    for i, req in enumerate(stream, start=1):
        ...
        print(
            f"\n[DIALOGUE SEMANTIC+VALIDITY-{STRICTNESS.upper()} BATCH {batch_idx} {i}/{len(stream)}] "
        ...
        print(f"GOLD STATE: {gold_state}")

        start = time.perf_counter()
        pred, hit, hit_gold_valid = runner.answer_semantic_plus_dialogue_validity(
            question=question,
            history=history,
        ...
        print(
            f"  dialogue semantic+validity ({STRICTNESS}) -> {pred!r} | "
            f"hit={hit} | hit_gold_valid={hit_gold_valid} | "
            f"answer_correct={answer_correct} | failed={failed}"
        )

        sleep_between_requests()

    output_json_path = f"tmp/dialogue_semantic_plus_validity_{STRICTNESS}_batch_{batch_idx}.json"

    payload = {
        "config": {
            "batch_idx": batch_idx,
            "batch_size": BATCH_SIZE,
            "policy": "semantic_plus_dialogue_validity",
            "strictness": STRICTNESS,
...
if __name__ == "__main__":
    ...
    run_dialogue_semantic_plus_validity_batch(
